...
def load_calibration_df() -> pd.DataFrame:
    ...
    # Make sure columns exist so appending records is easy
    cols = [
...
def get_or_calibrate_complexity_power(
    ...
    If not present, call calibrate.calibrate_complexity_power and append the row.
    Returns:
    ...
    # Need to calibrate
    print("\n=== No calibration found for (p, e) = "
          f"({p:.3f}, {e:.3f}) → running calibrate_complexity_power ===")
    ...
    # Build flat record compatible with CSV
    record = {
        "p": float(res.get("p", p)),
        "e": float(res.get("e", e)),
...
def run_offline_simulation(run_cfg: RunConfig, status_cb=None) -> SimulationResults:
    ...
    set_status("Checking / computing complexity exponent...")
